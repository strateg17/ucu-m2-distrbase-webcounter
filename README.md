# Web Counter Performance Playground
...
Сховище лічильника можна перемикати між оперативною пам'яттю та файловою системою змінною оточення `STORAGE_MODE` (`memory` або `file`). Для файлового режиму шлях до файлу задається `COUNTER_FILE` (за замовчуванням `./data/counter.txt`).
...
```bash
# Пам'ятний режим (за замовчуванням)
